...
locale max_poly_encoding_impl =
  ...
  where default_fun = "\<lambda>x. max_poly.MaxF" +
  verbose: pre_encoded_ord_algebra
  where less_eq = "(\<le>)" and less = "(<)" and I = max_poly.I and E = verbose.encoder.E
begin
...
sublocale encoder: encoding_impl
  where alist = "simplified_alist"
    and default_fun = "\<lambda>_. max_poly.MaxF" .
...
definition "check_less_term solver s t = IA.check_valid_formula solver (max_poly.less_via_IA (encode s) (encode t))"

definition "check_less_eq_term solver s t = IA.check_valid_formula solver (max_poly.le_via_IA (encode s) (encode t))"

definition "check_simple_arg_pos solver f n i = do {
  check (i < n) (showsl (STR ''bad argument number''));
  IA.check_valid_formula solver (max_poly.le_via_IA (Var i) (encoder.E f n))
}"

definition "check_ss_fun_arg solver f n i = do {
  check (i < n) (showsl (STR ''bad argument number''));
  IA.check_valid_formula solver (max_poly.less_via_IA (Var i)  (encoder.E f n))
}"

definition [simp]: "create_max_poly_rel_impl solver = \<lparr>
  rel_impl.valid = encoder.check_encoding,
  standard = succeed,
  desc = showsl_lit (STR ''max-polynomial interpretations:'') \<circ>
  ...
  top_refl = succeed,
  mono_af = empty_af,
  mono = (\<lambda> sig. error (showsl_lit (STR ''monotonicity of max-poly is not yet supported''))),
  ...
  not_sst = None, \<comment> \<open>TODO: add something here to support WPO with rule (2d);
     ...
     i.e., @{const max_poly.MaxF} in our case.\<close>
...
datatype 'f max_poly_impl = Max_Poly_Impl la_solver_type "(('f \<times> nat) \<times> (max_poly.sig, nat)term) list"

fun max_poly_rel_impl where
  "max_poly_rel_impl (Max_Poly_Impl type rp) = max_poly_encoding_impl.create_max_poly_rel_impl rp type" 


lemma max_poly_rel_impl: "rel_impl (max_poly_rel_impl mpi)"
  unfolding rel_impl_def 
proof (intro impI allI, goal_cases)
  case (1 U)
  obtain type rp where mpi: "mpi = Max_Poly_Impl type rp" by (cases mpi, auto)
  note 1 = 1[unfolded mpi, simplified]
  ...
  proof (rule exI[of _ ?S], intro exI[of _ ?NS] conjI impI 
    ...
    show "af_compatible (rel_impl.af ?rp) ?NS" 
      using af_compatible by (simp add: mpi)
    show "af_compatible (rel_impl.top_af ?rp) ?NS" 
      using af_compatible by (simp add: mpi)
    show "af_monotone (rel_impl.mono_af ?rp) ?S" 
      by (simp add: mpi empty_af)
    show "not_subterm_rel_info ?NS (rel_impl.not_wst ?rp)" 
      by (auto simp:simple_arg_pos_def mpi
        dest!:default_E 
        intro!: less_eq_termI le_max_list)
  qed (auto simp: mpi no_complexity_check_def check_less_term[OF enc] check_less_eq_term[OF enc])
qed
...
declare max_poly_encoding_impl.create_max_poly_rel_impl_def [code]
